...
| `loopRecords`                | `ReadonlyMap<string, LoopRecord>` | `undefined`  | Loop recordings keyed by IDENTITY (`structureRecordKey(ownerInstancePath, loopStructureId)` — see `docs/runner/executionRecordingDoc.md` › Record keys). Look records up with `resolveStructureRecord(loopRecords, step.loopStructureId, step.instancePath)`, never with a bare `.get(loopStructureId)`: two instances of one node group share their template's structure ids. Used to enrich the loop-context block with total iteration count and the iteration's loop condition value. |
